...
def Model(r,body,path=None):
    ...
    if body == 'Sun':
        if path == None:
            model       = np.genfromtxt('../propagate/models/struct_b16_agss09.dat')
        else:
        ...
        if path == None:    
            model       = np.genfromtxt('../propagate/models/EARTH_MODEL_PREM.dat')
        else:
            model       = np.genfromtxt(path)
...
def SunZenith(MJD,l_det):
    ...
    zenith  =  abs(delta-np.deg2rad(l_det))
    return zenith
...
def Pack(ch,DMm,mass_v):
    ...
    path = dirpath + "/../Pythia/no_EW/secluded/secluded/"
    flux_list   = {} 
    for i in range(6):
        files =  sorted(glob.glob(path+"{}_{:.1f}_{:.1f}_*_--{}.dat".format(ch,DMm,mass_v,i)),key=lambda s:key(s))
        flux_list[i]   = []
        for j in files:
    ...
    np.save(path+"{}_{:.1f}_{:.1f}.npy".format(ch,DMm,mass_v),flux)
    return flux
...
def IniFluxFunction(ch,DMm,process='ann',wimp_loc='Sun',path=None,secluded=False,mass_v=1000.):
    ...
    #filedir = dirpath + '/../production/data/'
    filedir = '/data/user/qliu/DM/DMFlux/production/data/'
    if secluded == False:
        f = []
        if path == None:    
            if DMm >= 500.:
                data =  h5py.File(filedir+'SpectraEW.hdf5','r')    
            elif DMm < 500.:        
                data  = h5py.File(filedir+'Spectra_noEW.hdf5','r')
            ...
            x    = data['x'][:]/factor
            for k in range(6):
                #print x.shape, mass[mass>=p_mass[ch]].shape, flux_data[:,k,:].shape
                #print mass[mass>=p_mass[ch]]
        ...
        if path      == None:
            filedir      = '/data/user/qliu/DM/DMFlux/Pythia/no_EW/secluded/secluded/'
            secluded_file= filedir+ch+'_{:.1f}_{:.1f}.npy'.format(DMm,mass_v)
            if os.path.exists(secluded_file):
                data  = np.load(filedir+ch+'_{:.1f}_{:.1f}.npy'.format(DMm,mass_v))
            else:
                data  = Pack(ch,DMm,mass_v)    
        else:
            data         = np.load(path)
...
def IniFlux(Enu,ch,DMm,process='ann',wimp_loc='Sun',secluded=False,r = None,mass_v=1000.,pathFlux=None,pathModel=None):
    ...
    if not secluded:
        ...
        flux = np.zeros((len(r),len(Enu)),dtype=[('nu_e','float'),('nu_mu','float'),
               ('nu_tau','float'),('nu_e_bar','float'),('nu_mu_bar','float'), ('nu_tau_bar','float')])
        for k in flux.dtype.names:
            rho     = np.array([Model(i,wimp_loc,path=pathModel) for i in r])
            if len(rho[rho>0.]) > 1:
...
def propagate(iniflux,Ein,Eout,location_ini,location_end,theta_12=33.82,theta_23=48.6,theta_13=8.60,delta_m_12=7.39e-5,delta_m_13=2.528e-3,delta=0.,interactions=True,xsec = None,secluded=False,r=0.0,mass_v=0.,zenith =0.,avg=True):
...
##############################Generate fluxes##################################
class NuFlux:
    def __init__(self,ch,DMm, nodes,Emin = 0.,Emax = None, bins = None, logscale=False, process='ann',theta_12=33.82,theta_23=48.6,theta_13=8.60,delta_m_12=7.39e-5,delta_m_13=2.528e-3,delta=0.,interactions=True,xsec=None,pathFlux=None, pathModel=None):
    ...
    def iniFlux(self,wimp_loc):
        ...
        wimp_loc   :    string_
                        location of neutrino production
        """
        return IniFlux(self.params['Ein'],self.ch,self.DMm,process=self.process,wimp_loc=wimp_loc,pathFlux=self.pathFlux,pathModel=self.pathModel) 
    ...
    def iniFlux_secluded(self,wimp_loc,mass_v,r):
        ...
        mass_v          :    float
                             mediator_mass in GeV
        """
        return IniFlux(self.params['Ein'],self.ch,self.DMm,process='decay',wimp_loc=wimp_loc,pathFlux=self.pathFlux,pathModel=self.pathModel,secluded=True,r=r,mass_v=mass_v)
