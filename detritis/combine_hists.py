...
if tp=="reg":
    ...
    for n in ns:
        nu_flux     += np.load("%s/partial_hists/ch%d_m%d_nu_%d_energy_delta_theta_hist_rescale.npy" %(data_path, ch, m, n))
        #if any_nan(nu_flux, "nu_flux"):
        if np.any(np.isnan(nu_flux)):
            bad_ones.append((ch, m, n, "nu"))
        nu_bar_flux += np.load("%s/partial_hists/ch%d_m%d_nuBar_%d_energy_delta_theta_hist_rescale.npy" %(data_path, ch, m, int(n)))
        if any_nan(nu_bar_flux, "nu_bar_flux"):
            bad_ones.append((ch, m, n, "nu_bar"))
    ...
    #np.save("%s/ch%d_m%d_nu_e_d_theta_hist.npy" % (data_path, ch, m), nu_flux)
    #np.save("%s/ch%d_m%d_nu_bar_e_d_theta_hist.npy" % (data_path, ch, m), nu_bar_flux)
    np.save("%s/ch%d_m%d_tot_e_d_theta_hist_rescale.npy" % (data_path, ch, m), tot_flux)
